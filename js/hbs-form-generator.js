...
class HBSFormGenerator {
    ...
    generateIdentitySection(container) {
        ...
        try {
            ...
            console.error(`Error generating identity section: ${error.message}`, error);
    ...
    getEnumValuesOrDefault(propertyName, defaultValues) {
        try {
            ...
            if (!property?.enum) {
                console.warn(`Property ${propertyName} not found or has no enum values in schema, using default values`);
                return defaultValues;
            }
            return property.enum;
        } catch (error) {
            console.warn(`Error getting enum values for ${propertyName}: ${error.message}`, error);
            return defaultValues;
        }
    }
    ...
    updateBuildOptions(gender, buildSelect) {
        ...
        let options = [];
        try {
            // Get options from schema based on gender
            if (this.schema?.gender_conditional_options) {
            ...
            console.error(`Error getting build options for gender ${gender}: ${error.message}`, error);
            options = gender === 'Identify as female' ? ["Default Female Build"] : ["Default Male Build"];
        }
    ...
    generateFacialFeaturesSection(container) {
        ...
        try {
            ...
            try {
                ...
                console.warn(`Using default eye modifiers: ${e.message}`);
            }
            ...
            console.error(`Error generating facial features section: ${error.message}`, error);
    ...
    generateHairSection(container) {
        ...
        try {
            ...
            console.error(`Error generating hair section: ${error.message}`, error);
    ...
    updateHairShadeOptions(colorGroup, shadeSelect) {
        ...
        try {
            ...
            console.error(`Error getting shade options for ${colorGroup}: ${error.message}`, error);
            options = defaultShades[colorGroup] || [];
        }
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (window.hbsApp && window.hbsApp.schema) {
            console.log("Using schema from H/**
 * HBS Form Generator
 * Generates form elements from the HBS schema
 */
class HBSFormGenerator {
