...
function generateT2IPrompt(characterData, options = {}) {
  ...
  promptParts.push(ageDescription ? `${ageDescription} ${identity}` : identity);
  ...
  if (includeDetails) {
    ...
    // Add eye details
    const eyeModifiers = params.eyes.modifiers?.map(m => m.toLowerCase()).join(" ") || "";
    promptParts.push(`${eyeModifiers && `${eyeModifiers} `}${params.eyes.shape.toLowerCase()} ${params.eye_color.toLowerCase()} eyes`);

    // Add eyebrows if distinctive
    if (params.eyebrows.modifiers?.length > 0) {
  ...
  if (params.hair_style !== "Bald") {
    ...
    if (params.hair_length !== "Buzz Cut") {
      ...
      if (params.hair_volume !== "Medium Volume") {
        hairDesc += `${params.hair_volume.replace(" Volume", "").toLowerCase()} volume `;
      }
...
console.log(prompt);
// Result: "professional portrait photo of a young adult Latino woman with athletic build, with sun-kissed tan skin, heart face, upturned almond brown eyes, groomed tapered arched eyebrows, narrow-...
*/
